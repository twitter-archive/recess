...
var RECESS = require('./core')
  ...
  , checkstyle = require('./reporters/checkstyle.js')

// define main export
module.exports = function (paths, options, callback) {

  var option, message, reporter, i, instances = []
  ...
  // if checkstyle reporting, disable cli, compile, and compress
  if (options.report && options.report == 'checkstyle') {
    reporter = new checkstyle()
    options.cli = options.compile = options.compress = false
  }
  ...
  function recess(init, path, err) {
    if (path = paths.pop()) {
      return instances.push(new RECESS(path, options, recess, reporter))
    }
    ...
    reporter && reporter.endReport() || reporter.flush()
