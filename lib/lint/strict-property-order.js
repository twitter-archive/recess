...
module.exports = function (def, data) {
  ...
  sortedRules = _.sortBy(cleanRules, function (rule) {
    ...
    // console.log(util.inspect(rule, null, 10));
    // strip vendor prefix and hack prefix from rule name to find root
    // _.forEach(rule.name, function (ruleName) {
    //   root = ruleName.value.replace(VENDOR_PREFIX, '').replace(HACK_PREFIX, '');
    // });
  ...
  extract = sortedRules.map(function (rule) {
    ...
    var value = (typeof rule.value === 'string' ? rule.value : rule.value.toCSS({}));
    console.log(value)
