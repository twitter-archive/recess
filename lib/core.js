...
, log: function (str, force) {
    ...
    if ((!this.options.quiet && !this.options.compile) || force) {
        ...
        .parse(this.data, function (err, tree) {
          ...
          if (err) {
            ...
            err.extract && err.extract.forEach(function (line, index) {
              that.log(util.padLine(err.line + index) + line, true)
            })
...
, validateStatus: function (failed) {
    ...
    if (failed) {
      ...
      // log file overview
      this.log('FILE: ' + this.path.cyan, true)
      this.log('STATUS: ' + 'Busted'.magenta, true)
