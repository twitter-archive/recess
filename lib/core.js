...
function RECESS(path, options, callback, reporter) {
  this.path = path
  this.output = []
  this.errors = []
      ...
      new less.Parser(options)
        ...
        .parse(this.data, function (err, tree) {
          ...
          if (err) {
            ...
            if (that.reporter) {
              that.reporter.startFile(err.filename)
              that.reporter.logError(err)
              that.reproter.endFile()
            }
...
, validateStatus: function (failed) {
    ...
    // start reporting errors of a file
    this.reporter && this.reporter.startFile(this.path)
      ...
      this.definitions.forEach(function (def) {
        ...
        def.errors
          ...
          && def.errors.forEach(function (err) {
               ...
               // report error
               that.reporter && that.reporter.logError(err)
             })
      })
    ...
    // end reporting errors of a file
    this.reporter && this.reporter.endFile()
